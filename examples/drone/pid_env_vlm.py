        ...
        while not all(completed) and step < max_steps:
            ...
            for i, (drone, controller) in enumerate(zip(self.drones, self.controllers)):
                ...
                state = {
                    "drone_id": i,
                    "step": step,
                    "target": list(target), # target assined to current drone DONT USE
                    "position": pos.tolist(),
            ...
            # image to base64 for llm input
            base64_image = base64.b64encode(bgr).decode('utf-8')
            
            # ------------------------------------
                                         
            def openai_api_call(self, base64_image, drone_info_list):
                # Use OpenAI API to analyze the picture
                pass
            
            # LLM OUPUT:
            # 1. way-points
            # 2. pre donne -> target [drone_1: target_1, drone_2: target_2, ...]
            # 3. reasoning 
            ...
            middle_target_list = parse_llm_output(openai_api_call(base64_image))
            for i, target in enumerate(middle_target_list):
                self.target_markers[i].set_pos(target)
